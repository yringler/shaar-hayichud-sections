...
/**
 * Extract translation text from a children array.
 * Looks for a <translation> element and returns its normalized text content.
 *
 * @param {Array<Record<string, unknown>>} children
 * @returns {string} translation text, or "" if none
 */
function getTranslationText(children) {
  for (const node of children) {
    if ("translation" in node) {
      const kids = /** @type {Array<Record<string, unknown>>} */ (node["translation"]) ?? [];
      const raw = kids.map((k) => k["#text"] ?? "").join("");
      const normalized = raw.replace(/\s+/g, " ").trim();
      if (normalized) return normalized;
    }
  }
...
function renderChildren(nodes, depth, prefix, showTranslations) {
  ...
  return sections
    .map((node, idx) => {
      ...
      if (showTranslations) {
        const translation = getTranslationText(children);
        if (translation) {
          html += `<div class="translation">${escapeHtml(translation)}</div>\n`;
        }
      }
 ...
 */
async function renderXmlToHtml(xmlContent, data) {
  /** @type {Array<Record<string, unknown>>} */
  ...
  return renderChildren(rootChildren, 1, "", showTranslations);
}
